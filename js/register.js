...
// Firebase Imports
// Pastikan versi SDK ini cocok dengan yang Anda gunakan di login.js
...
// --- Logic Form Submission Utama dengan Firebase Authentication & Firestore ---
const registerForm = document.getElementById('registerForm');

// Pastikan elemen registerForm ada di HTML
if (registerForm) {
    registerForm.addEventListener('submit', async (e) => {
        ...
        // Tampilkan loading (sebelum validasi awal agar segera terlihat)
        if (submitBtn) {
            submitBtn.innerHTML = '<i class="fas fa-spinner fa-spin"></i> Creating account...';
            submitBtn.disabled = true;
        }

        // Validasi awal (sebelum interaksi dengan Firebase)
        ...
        try {
            ...
            await setDoc(doc(db, "users", uid), {
                username: username,
                email: email,
                role: "user", // default user, nanti Anda bisa ubah manual jadi admin di Firebase Console
                createdAt: serverTimestamp() // Menggunakan serverTimestamp() yang diimpor
            });
